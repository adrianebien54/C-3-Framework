#!/bin/bash
# CSRNet — bbox/2 sigma — all resolutions — 4 LRs × 600 epochs each
#
# Sigma table (bbox/2 rule):
#   49x33      → σ=2  → datasets/Tenebrio/49x33_s2
#   97x65      → σ=3  → datasets/Tenebrio/97x65_s3
#   193x130    → σ=6  → datasets/Tenebrio/193x130_s6
#   386x260    → σ=11 → datasets/Tenebrio/386x260
#   772x519    → σ=24 → datasets/Tenebrio/772x519_s24
#   1544x1038  → σ=48 → datasets/Tenebrio/1544x1038_s48
#
# Total: 6 resolutions × 4 LRs = 24 sequential jobs.
#
# Usage: nohup bash run_csrnet_allres_bbox2_sweep.sh > /tmp/csrnet_allres_bbox2.log 2>&1 &
set -e
...
echo ""
echo "════════════════════════════════════════════════════════"
echo "[$(date)] CSRNet bbox/2 all-resolution sweep — START"
echo "Resolutions: $RESOLUTIONS"
echo "LRs: $LR_LIST"
echo "════════════════════════════════════════════════════════"

for RES in $RESOLUTIONS; do
    DIR="${TENEBRIO}/${BBOX2_DIR[$RES]}"
...
echo ""
echo "════════════════════════════════════════════════════════"
echo "[$(date)] ALL DONE — CSRNet bbox/2 all-resolution sweep"
echo "════════════════════════════════════════════════════════"
echo ""
