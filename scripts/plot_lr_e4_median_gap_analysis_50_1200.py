...
def main():
    ...
    if divergence_epoch is not None:
        axs[0].axvline(divergence_epoch, color='red', linestyle='--', linewidth=2.5,
                       label=f'divergence epoch ~ {divergence_epoch}')
    axs[0].set_ylabel('Gap', fontsize=12)
    axs[0].set_title('LR 1e-4: Epochs 50-1200 — Gap Analysis (Median + Savitzky-Golay)', fontsize=13)
    ...
    if divergence_epoch is not None:
        axs[1].axvline(divergence_epoch, color='red', linestyle='--', linewidth=2.5,
                       label=f'divergence epoch ~ {divergence_epoch} (20+ consecutive positive)')
    axs[1].set_xlabel('Epoch', fontsize=12)
    axs[1].set_ylabel('Gap Derivative', fontsize=12)
    axs[1].legend(fontsize=10, loc='upper right')
    axs[1].grid(True, alpha=0.3)

    out = os.path.join('exp', 'tenebrio_batch1_1200ep', 'checkpoints',
                       'lr_1e-4_epochs50-1200_median_gap_analysis.png')
    ...
    if divergence_epoch is not None:
        print(f'Detected divergence epoch (median-filtered, 20+ consecutive positive derivative): {divergence_epoch}')
    else:
        print('No sustained positive divergence point detected')
