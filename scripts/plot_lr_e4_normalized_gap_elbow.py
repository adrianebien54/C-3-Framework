...
def compute_smoothed_curves(train_series, val_series, med_kernel=51, savgol_window=51):
...
def main():
    ...
    # Apply smoothing
    train_filtered, val_filtered = compute_smoothed_curves(train, val, med_kernel=51, savgol_window=51)
    ...
    axs[0].plot(epochs, train_norm, label='train loss (normalized)', color='C0', linewidth=2)
    axs[0].plot(epochs, val_norm, label='val loss (normalized)', color='C1', linewidth=2, linestyle='--')
    axs[0].set_ylabel('Normalized Loss (0-1)', fontsize=12)
    axs[0].set_title('LR 1e-4: Epochs 50-1200 — Normalized Smoothed Curves', fontsize=13)
    ...
    out = os.path.join('exp', 'tenebrio_batch1_1200ep', 'checkpoints',
                       'lr_1e-4_epochs50-1200_normalized_gap_elbow.png')
    ...
    print(f'  Gap at elbow: {gap.iloc[np.where(epochs == elbow_epoch)[0][0]]:.6f}')
