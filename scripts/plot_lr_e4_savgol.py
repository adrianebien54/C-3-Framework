...
def apply_savgol(series, window=51, polyorder=3):
    """Apply Savitzky-Golay filter to smooth the series."""
...
def main():
    ...
    train_filtered = apply_savgol(train, window=101, polyorder=2)
    val_filtered = apply_savgol(val, window=101, polyorder=2)

    fig, ax = plt.subplots(figsize=(14, 6))

    ax.plot(epochs, train_filtered, label='train loss (Savitzky-Golay)', color='C0', linewidth=2)
    ax.plot(epochs, val_filtered, label='val loss (Savitzky-Golay)', color='C1', linewidth=2, linestyle='--')

    ax.set_xlabel('Epoch', fontsize=12)
    ax.set_ylabel('Loss (MSE)', fontsize=12)
    ax.set_title('LR 1e-4: Epochs 50-1200 — Train vs Val Loss (Savitzky-Golay Filtered)', fontsize=13)
    ax.legend(fontsize=11, loc='upper right')
    ax.grid(True, alpha=0.3)

    out = os.path.join('exp', 'tenebrio_batch1_1200ep', 'checkpoints',
                       'lr_1e-4_epochs50-1200_savgol_w101_p2.png')
    ...
    print(f'Savitzky-Golay filter: window=101, polyorder=2')
