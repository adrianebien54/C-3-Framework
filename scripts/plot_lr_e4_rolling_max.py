...
def apply_rolling_max(series, window=51):
    """Apply rolling maximum filter to smooth the series."""
    return series.rolling(window=window, center=True, min_periods=1).max()
...
def main():
    ...
    # Apply rolling maximum filter
    train_filtered = apply_rolling_max(train, window=51)
    val_filtered = apply_rolling_max(val, window=51)

    fig, ax = plt.subplots(figsize=(14, 6))

    ax.plot(epochs, train_filtered, label='train loss (rolling maximum)', color='C0', linewidth=2)
    ax.plot(epochs, val_filtered, label='val loss (rolling maximum)', color='C1', linewidth=2, linestyle='--')

    ax.set_xlabel('Epoch', fontsize=12)
    ax.set_ylabel('Loss (MSE)', fontsize=12)
    ax.set_title('LR 1e-4: Epochs 50-1200 — Train vs Val Loss (Rolling Maximum)', fontsize=13)
    ax.legend(fontsize=11, loc='upper right')
    ax.grid(True, alpha=0.3)

    out = os.path.join('exp', 'tenebrio_batch1_1200ep', 'checkpoints',
                       'lr_1e-4_epochs50-1200_rolling_max.png')
    ...
    print(f'Rolling maximum filter: window=51')
